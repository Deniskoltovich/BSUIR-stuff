from lab3_lib.lab2copy import Context
...
class Minimizer:
    ...
    def __init__(self, formula: str = None, mode= 'DNF', truth_table = None, vars = None):
        self.mode = mode
        self.context = Context(formula) if formula else None
        self.vars = vars if vars else self.context.vars
    ...
    def karnough_method(self):
        if self.table:
            return self.process_table(self.table)
    ...
    def self_table_minim(self):
        return [self.vars[0], '(!q1*)' + ' & ' +  self.vars[0], 'v & (!q2*) & (!q1*)']
