...
std::pair<std::string, std::string> Searcher::process_logicalOP(const std::string& op,const std::string& current) {
...
std::vector<Element*> Searcher::recurion_check(const std::string& current, const std::string& logical_op) {
	...
	std::string left = process_logicalOP(logical_op, current).first;
	std::string right = process_logicalOP(logical_op, current).second;
