...
void xml::Parser::process()
{
	for (nextWord_inStream(); !eof; nextWord_inStream()) {
		in_file->peek() == '<' ? in_file->get(), processTag() : processText();
	}
}

void xml::Parser::nextWord_inStream() //достаем слово и проверяем не конец ли файла
{
	for (;;) {
		char next = in_file->peek();//не извлекает символ из потока
		if (next == '\n') { in_file->get(); continue; };
		if (in_file->eof()) break;
		if (next != ' ') return;
		in_file->get();//извлечение из потока
	}
	eof = true;
}
...
void xml::Parser::processTag()//проверяем открывается тег, закрывается или это инструкция обработки
{
	...
	if (next == '?') {
		...
		in_file->get();
		if (getStringBefore('>') == tags.top()->value) tags.pop();//тегс топ - недавно открывшийся тэг
	}
	else OpenTag();
}
...
void xml::Parser::OpenTag()
{
	...
	if (document->root) {//если не пустой док
		tagElement->parent = tags.top();//указываем родителя тэга
		tags.top()->children.push_back(tagElement);//родителю говорим кто сын
		tags.push(tagElement);//закидываем в тэгс
	}
	else {
